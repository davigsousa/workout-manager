...
const getManifestPath = () =>
    ({
        vercel: '../.vercel_build_output/functions/node/server/chunks/app/client.manifest.mjs',
...
const buildPwa = async() => {
    // filter pages/!* to include them on sw precache manifest to work offline
    const { default: entries } = await import(getManifestPath())
    console.log(`Fetching manifests for ${process.env.NITRO_PRESET}` || 'node-server')
    ...
    const config = await resolveConfig({ plugins: [VitePWAPlugin] }, 'build', 'production')
